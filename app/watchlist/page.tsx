...
export default function WatchlistPage() {
  ...
  async function handleResolve(id: string, outcome: boolean) {
    ...
    // The side you hold resolves to 1 if it wins, 0 if it loses. P&L is the return
    // on the entry price you paid for that side.
    const win = (entry.position === 'YES') === outcome
    const resolveSide = win ? 1 : 0
    const pnl = entry.entry_price
      ? ((resolveSide - entry.entry_price) / entry.entry_price) * 100
      : 0
